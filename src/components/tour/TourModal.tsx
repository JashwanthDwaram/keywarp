...
export const TourModal: React.FC<TourModalProps> = ({ isOpen, onClose, onTabChange }) => {
  ...
  return (
    ...
    <aside
      ...
      className="fixed bottom-4 sm:bottom-6 right-4 sm:right-6 z-50 w-[calc(100vw-2rem)] sm:w-[420px] pointer-events-auto animate-in slide-in-from-bottom-5 fade-in duration-200"
    >
      <div className="rounded-lg bg-surface border border-accent/50 shadow-2xl p-4 sm:p-5 space-y-3 font-sans select-none backdrop-blur-md">
        {/* Top Header Row */}
        <div className="flex items-center justify-between gap-2 border-b border-ink-400/10 pb-2.5">
          <div className="flex items-center gap-2 min-w-0">
            <div className="w-7 h-7 rounded bg-accent/10 border border-accent/30 flex items-center justify-center text-accent shrink-0">
              {step.icon}
            </div>
        ...
        <div className="p-2 rounded bg-bg/80 border border-ink-400/15 flex items-center gap-2 text-[11px] font-mono text-ink-400">
          ...
          <div className="flex items-center gap-1.5">
            {currentStep > 0 && (
              <button
                type="button"
                onClick={handlePrev}
                className="px-2.5 py-1 rounded text-xs font-medium text-ink-400 hover:text-ink-100 hover:bg-bg/50 transition-colors flex items-center gap-1 cursor-pointer"
              >
            ...
            <button
              type="button"
              onClick={handleNext}
              className="px-3.5 py-1.5 rounded bg-accent text-accent-contrast text-xs font-semibold hover:opacity-90 active:scale-95 transition-all flex items-center gap-1.5 shadow-xs cursor-pointer"
            >
              <span>{currentStep === steps.length - 1 ? 'Start Typing' : 'Next Step'}</span>
